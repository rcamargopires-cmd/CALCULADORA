...
const normalizedVehicle=(raw:any)=>{
  ...
  return {
    ...
    chassis:String(first(root.chassi,root.chassis,root.vin,root.numeroChassi,root.numero_chassi)||''),
    vehicleType:String(first(root.tipo,root.tipoVeiculo,root.tipo_veiculo,root.vehicleType,root.especieTipo,root.especie_tipo)||''),
    ...
    try{
      ...
      if(response.ok&&raw&&Number(raw.codigo)===1){
        ...
        return res.status(200).json({
          ...
          chassis:String(info.chassi||info.chassis||info.vin||''),
          vehicleType:String(info.tipo||info.tipo_veiculo||info.especie_tipo||''),
