...
const SellerDashboard: React.FC<Props> = ({ currentUser, history, onStartNewCalculation }) => {
  ...
  // Todos os hooks ficam acima de qualquer retorno condicional.
  // Isso evita o erro "Rendered more hooks than during the previous render"
  // quando o mapa passa de não vinculado para vinculado.
  ...
  if (loading) {
    return (
      <div className="grid min-h-[50vh] place-items-center text-sm text-zinc-500">
        Carregando seu My Performance...
      </div>
    );
  }
      ...
      <div className="pb-24 md:pb-12 space-y-6 animate-fade-in">
        <section>
          ...
          <h2 className="mt-1 text-3xl font-semibold tracking-tight text-white md:text-4xl">
            Bom dia, {currentUser.name?.split(' ')[0] || 'Vendedor'}.
          </h2>
          <p className="mt-2 text-zinc-400">
            Seu acesso está protegido. Nenhum dado de outros vendedores é exibido aqui.
          </p>
        </section>
        ...
        <section className="rounded-[32px] border border-white/10 bg-gradient-to-br from-zinc-800 via-zinc-900 to-black p-7 md:p-9">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-sm leading-6 text-zinc-400">
              Quando o gestor atualizar o Mapa de Performance, o DealMaster identifica seu usuário e libera somente os seus indicadores neste painel.
            </p>
            <button onClick={onStartNewCalculation} className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-black">
              Abrir DealGuard <ArrowRight size={17}/>
            </button>
          </div>
        </section>
      </div>
  ...
  if (projection < goal) {
    actionPlan.push(
      `Recupere o ritmo: sua projeção é ${projection.toFixed(1)} para uma meta de ${goal}. Faltam ${missing} venda(s), cerca de ${dailyNeeded.toFixed(2)} por dia trabalhado restante.`,
    );
  }
  if (capture < goals.capture) {
    actionPlan.push(
      `Captura em ${capture.toFixed(1)}%. Priorize oportunidades com veículo na troca para buscar a meta de ${goals.capture}%.`,
    );
  }
  if (actual > 0 && margin < goals.margin) {
    actionPlan.push(
      `Margem MC em ${margin.toFixed(1)}%. Proteja rentabilidade nas próximas negociações para voltar à meta de ${goals.margin}%.`,
    );
  }
  if (metrics.flowTotal > 0 && metrics.evaluations === 0) {
    actionPlan.push(
      `Você tem ${metrics.flowTotal} oportunidade(s) no fluxo e nenhuma avaliação registrada. Transforme fluxo em avaliação antes de mexer em preço.`,
    );
  }
  if (openDeals > 0 && missing > 0) {
    actionPlan.push(
      `Existem ${openDeals} negociação(ões) suas no DealGuard. Priorize avanço e fechamento das oportunidades mais maduras.`,
    );
  }
  if (!actionPlan.length) {
    actionPlan.push('Você está dentro do ritmo dos principais objetivos. Mantenha atividade, proteja margem e continue alimentando o funil.');
  }
  ...
  return (
    <div className="pb-24 md:pb-12 space-y-6 md:space-y-8 animate-fade-in">
      <section className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <span className="w-fit rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs text-zinc-500">
          Dados privados do seu login
        </span>
      </section>
        ...
        <div className="grid gap-7 lg:grid-cols-[1.25fr_.75fr] lg:items-end">
          <div>
            ...
            <div className="flex items-end gap-3">
              <span className="text-6xl font-semibold tracking-[-0.06em] text-white md:text-7xl">{actual}</span>
              <span className="pb-2 text-xl text-zinc-500">de {goal}</span>
            </div>
            <div className="mt-6 h-2.5 overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-white" style={{ width: `${Math.min(actual / Math.max(goal, 1) * 100, 100)}%` }}/>
            </div>
          ...
          <button onClick={onStartNewCalculation} className="flex min-h-24 items-center justify-between rounded-[26px] bg-white px-5 py-5 text-left text-black">
            <div>
              <span className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">DealGuard</span>
              <span className="block text-lg font-semibold">Nova negociação</span>
              <span className="mt-1 block text-sm text-zinc-500">Proteja a margem antes de fechar.</span>
            </div>
      ...
      <section className={`rounded-[30px] border p-6 md:p-7 ${statusGood ? 'border-emerald-500/20 bg-emerald-500/[0.07]' : 'border-amber-400/20 bg-amber-400/[0.07]'}`}>
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-zinc-400">Action Center pessoal</p>
            <h3 className="mt-1 text-xl font-semibold text-white">Seu plano de ação de hoje</h3>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
        <div className="rounded-[30px] border border-white/10 bg-white/[0.035] p-5 md:p-7">
          ...
          <div className="mt-6 grid grid-cols-[1fr_auto_1fr_auto_1fr] items-center gap-2">
            <FunnelStep label="Fluxo" value={`${metrics.flowTotal}`}/><ArrowRight size={16} className="text-zinc-700"/>
            <FunnelStep label="Pedidos" value={`${metrics.orders}`}/><ArrowRight size={16} className="text-zinc-700"/>
            <FunnelStep label="Vendas" value={`${actual}`}/>
          </div>
          <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <Small label="Passagens" value={`${metrics.passages}`}/>
            <Small label="Pedido %" value={pct(metrics.orderPercent)}/>
            <Small label="Fechamento" value={pct(closingRate)}/>
            <Small label="Caixa d'água" value={`${metrics.pipeline}`}/>
          </div>
        </div>
        ...
        <div className="rounded-[30px] border border-white/10 bg-white/[0.035] p-5 md:p-7">
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <Small label="Trab. período" value={`${metrics.workInPeriod}`}/>
            <Small label="Contatos/dia" value={Number(metrics.avgContactsPerDay || 0).toFixed(2)}/>
            <Small label="MC por carro" value={formatCurrency(metrics.marginPerCar)}/>
            <Small label="Compra adicional" value={`${metrics.additionalPurchase}`}/>
            <Small label="Negociações DealGuard" value={`${openDeals}`}/>
            <Small label="Projeção" value={projection.toFixed(1)}/>
          </div>
        </div>
      </section>
...
const Metric = ({ icon, label, value, hint }: { icon: React.ReactNode; label: string; value: string; hint: string }) => (
  <div className="rounded-[26px] border border-white/10 bg-white/[0.035] p-5">
    <div className="mb-5 grid h-9 w-9 place-items-center rounded-2xl bg-white/[0.06] text-zinc-300">{icon}</div>
...
const Benchmark = ({ icon, label, current, target }: { icon: React.ReactNode; label: string; current: string; target: string }) => (
  <div className="rounded-[22px] border border-white/10 bg-white/[0.03] p-4">
    ...
    <div className="mt-3 flex items-end justify-between gap-3">
      <span className="text-xl font-semibold text-white">{current}</span>
      <span className="text-xs text-zinc-600">meta {target}</span>
    </div>
  </div>
);
