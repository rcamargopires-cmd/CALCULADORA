        ...
        <div className="p-5 md:p-6">
          ...
          {mode==='new' && <div>
            ...
            {stage==='research' && <div>
              ...
              <div className="grid gap-3 md:grid-cols-2">{researchSteps.map(step=><div key={step.key} className={`rounded-2xl border p-4 ${completed[step.key]?'border-emerald-400/20 bg-emerald-400/[0.05]':'border-white/10 bg-white/[0.02]'}`}><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-white">{step.label}</p><p className="mt-1 text-xs text-zinc-500">{step.hint}</p></div><button onClick={()=>toggle(step.key)} className={completed[step.key]?'text-emerald-300':'text-zinc-700'}><CheckCircle2 size={22}/></button></div><div className="mt-4 flex gap-2"><button onClick={()=>window.open(step.url,'_blank','noopener,noreferrer')} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-bold text-black"><ExternalLink size={14}/> ABRIR</button>{step.key==='detran'?<><button onClick={()=>copyText(form.plate,'detran-plate')} className="flex items-center justify-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-xs font-bold text-zinc-300"><Clipboard size={14}/>{copied==='detran-plate'?'COPIADO':'COPIAR PLACA'}</button><button onClick={()=>copyText(form.renavam,'detran-renavam')} className="flex items-center justify-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-xs font-bold text-zinc-300"><Clipboard size={14}/>{copied==='detran-renavam'?'COPIADO':'COPIAR RENAVAM'}</button></>:<button onClick={()=>copyText(step.copy(form),step.key)} className="flex items-center justify-center gap-2 rounded-xl border border-white/10 px-3 py-2 text-xs font-bold text-zinc-300"><Clipboard size={14}/>{copied===step.key?'COPIADO':'COPIAR'}</button>}</div></div>)}</div>
