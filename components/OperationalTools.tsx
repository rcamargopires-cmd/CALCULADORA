...
const OperationalTools: React.FC = () => {
  ...
  const resolveContext = async (profile: User) => {
    ...
    setCompanies(availableCompanies); setStores(availableStores);
  ...
  useEffect(() => onAuthStateChanged(auth, async firebaseUser => {
    ...
    try { const profile = await userService.getUser(firebaseUser.email); if (profile?.status !== 'active') { setUser(null); return; } await resolveContext(profile); setUser(profile); } catch { setUser(null); }
  }), []);
