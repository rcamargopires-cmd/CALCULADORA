...
type MarketLevel = 'high' | 'medium' | 'low' | 'unknown';
...
const marketLevelLabel=(value:MarketLevel)=>value==='high'?'ALTA':value==='medium'?'MÉDIA':value==='low'?'BAIXA':'AGUARDANDO';
const turnoverLabel=(score:number)=>score>=78?'BAIXO':score>=58?'MODERADO':'ALTO';
...
const MarketIQ:React.FC<Props>=({currentUser,storeName})=>{
 ...
 const[marketSupply,setMarketSupply]=useState<MarketLevel>('unknown');
 const[marketDemand,setMarketDemand]=useState<MarketLevel>('unknown');
 const[marketCompetition,setMarketCompetition]=useState<MarketLevel>('unknown');
 const[marketPressureReason,setMarketPressureReason]=useState('');
 ...
 useEffect(()=>{
   const openEvaluation=(event:Event)=>{
     ...
     setMarketBalance('');
     setMarketSupply('unknown');
     setMarketDemand('unknown');
     setMarketCompetition('unknown');
     setMarketPressureReason('');
     setOpen(true);
   };
   const applyMarketPressure=(event:Event)=>{
     const detail=(event as CustomEvent).detail||{};
     const balance=clamp(Number(detail.balanceScore)||0,0,100);
     if(balance>0){
       setMarketBalance(String(Math.round(balance)));
       setLiquidity(balance>=78?'high':balance>=55?'medium':'low');
     }
     const supply=String(detail.supply||'unknown') as MarketLevel;
     const demand=String(detail.demand||'unknown') as MarketLevel;
     const competition=String(detail.competition||'unknown') as MarketLevel;
     if(['high','medium','low'].includes(supply))setMarketSupply(supply);
     if(['high','medium','low'].includes(demand))setMarketDemand(demand);
     if(['high','medium','low'].includes(competition))setMarketCompetition(competition);
     if(['up','stable','down'].includes(String(detail.priceTrend||'')))setTrend(String(detail.priceTrend) as Trend);
     setMarketPressureReason(String(detail.rationale||''));
   };
   window.addEventListener('motyq:marketiq-open-request-v2',openEvaluation as EventListener);
   window.addEventListener('motyq:marketiq-market-pressure-applied',applyMarketPressure as EventListener);
   return()=>{
     window.removeEventListener('motyq:marketiq-open-request-v2',openEvaluation as EventListener);
     window.removeEventListener('motyq:marketiq-market-pressure-applied',applyMarketPressure as EventListener);
   };
 },[]);

 const calc=useMemo(()=>{
   const m=num(market),f=num(fipe),low=num(marketLow),high=num(marketHigh),actualKm=num(km),expKm=Math.max(1,num(expectedKm));
   const prep=Object.values(costs).reduce<number>((s,v)=>s+num(v),0);
   ...
   const manualLiquidityScore=liquidity==='high'?92:liquidity==='medium'?72:48;
   const measuredMarketBalance=num(marketBalance);
   const marketBalanceScore=measuredMarketBalance?clamp(measuredMarketBalance,25,98):manualLiquidityScore;
   const marketPriceAdjustment=measuredMarketBalance?clamp((marketBalanceScore-65)*.001,-.045,.015):0;
   const base=m||f||0;
   let sell=base*(1+conditionAdjustment+kmAdjustment+trendAdjustment+marketPriceAdjustment);
   if(low&&high)sell=clamp(sell,low*.94,high*1.01);
   ...
   const liquidityRisk=measuredMarketBalance?0:liquidity==='high'?.008:liquidity==='low'?.035:.018;
   const supplyDemandRisk=measuredMarketBalance?(marketBalanceScore<40?.035:marketBalanceScore<55?.024:marketBalanceScore<70?.012:marketBalanceScore<85?.006:.003):0;
   const trendRisk=trend==='down'?.025:trend==='up'?.005:.012;
   const conditionRisk=weighted<65?.025:weighted<78?.015:.007;
   const riskReserve=sell*(liquidityRisk+supplyDemandRisk+trendRisk+conditionRisk);
   ...
   const score=Math.round(clamp(weighted*.20+kmScore*.15+marketBalanceScore*.30+trendScore*.15+economicsScore*.20,0,100));
   return{prep,weighted,kmRatio,kmScore,sell,capitalCost,riskReserve,recommendedBuy,safeBuy,aggressiveBuy,limitBuy,marginAtRecommended,marginPctReal,score,base,marketBalanceScore,marketPriceAdjustment,supplyDemandRisk};
 },[market,fipe,marketLow,marketHigh,km,expectedKm,targetMargin,expectedDays,capitalRate,liquidity,trend,marketBalance,body,interior,tires,mechanical,history,costs]);

 const diagnosis=useMemo(()=>{
   if(!calc.base)return 'Informe FIPE ou mercado observado para gerar a recomendação financeira.';
   const pressure=num(marketBalance)
     ? ` Oferta ${marketLevelLabel(marketSupply).toLowerCase()}, demanda estimada ${marketLevelLabel(marketDemand).toLowerCase()} e equilíbrio de mercado ${Math.round(calc.marketBalanceScore)}/100.`
     : ' Rode o MarketScan para incorporar oferta, demanda estimada e pressão competitiva ao score.';
   if(calc.score>=80)return `Boa oportunidade.${pressure} Estado ${conditionLabel(calc.weighted)}, KM score ${Math.round(calc.kmScore)}/100 e preparação estimada em ${money(calc.prep)}.`;
   if(calc.score>=65)return `Compra viável com disciplina.${pressure} O ponto de equilíbrio está próximo de ${money(calc.recommendedBuy)}. Evite ultrapassar ${money(calc.limitBuy)} sem justificativa comercial.`;
   if(calc.score>=50)return `Atenção.${pressure} Há fatores que pressionam margem, giro ou preparação. Negocie abaixo de ${money(calc.recommendedBuy)} para recuperar proteção.`;
   return `Risco elevado.${pressure} O conjunto de condição, mercado, giro e custos sugere compra defensiva ou direcionamento para repasse.`;
 },[calc,marketBalance,marketSupply,marketDemand]);
  ...
  {open&&<div className="fixed inset-0 z-[590] overflow-y-auto bg-black/80 p-3 backdrop-blur-md md:p-5" onClick={()=>setOpen(false)}>
   <div className="mx-auto max-w-[1500px] rounded-[28px] border border-white/10 bg-[#101315] text-white shadow-2xl" onClick={e=>e.stopPropagation()}>
    <header className="sticky top-0 z-20 flex items-start justify-between rounded-t-[28px] border-b border-white/10 bg-[#101315]/95 p-5 backdrop-blur md:px-6 md:py-5">
     <div><p className="text-[10px] font-black uppercase tracking-[.18em] text-cyan-300">MOTYQ MARKETIQ · V3</p><h2 className="mt-1 text-2xl font-semibold">Avaliação & Precificação Inteligente</h2><p className="mt-1 text-sm text-zinc-500">{storeName} · veículo + mercado + oferta × demanda + condição + decisão</p></div>
    ...
    <div className="grid gap-5 p-4 md:p-6 xl:grid-cols-[1.25fr_.8fr_.72fr]">
     ...
     <div className="space-y-5">
      <section className="rounded-2xl border border-white/10 bg-white/[.025] p-4">
       ...
       <div className="mt-4 grid grid-cols-2 gap-3">{metric('Oferta',marketLevelLabel(marketSupply),marketSupply==='high'?'text-red-200':marketSupply==='low'?'text-emerald-300':'text-amber-200')}{metric('Demanda estimada',marketLevelLabel(marketDemand),marketDemand==='high'?'text-emerald-300':marketDemand==='low'?'text-red-200':'text-amber-200')}{metric('Pressão competitiva',marketLevelLabel(marketCompetition),marketCompetition==='high'?'text-red-200':marketCompetition==='low'?'text-emerald-300':'text-amber-200')}{metric('Equilíbrio mercado',num(marketBalance)?`${Math.round(calc.marketBalanceScore)}/100`:'Aguardando MarketScan',num(marketBalance)?(calc.marketBalanceScore>=70?'text-emerald-300':calc.marketBalanceScore>=55?'text-amber-200':'text-red-200'):'text-zinc-500')}</div>
       {marketPressureReason&&<div className="mt-3 rounded-xl border border-cyan-300/15 bg-cyan-300/[.035] p-3 text-xs leading-5 text-zinc-300">{marketPressureReason}</div>}
       <div className="mt-4 space-y-2 rounded-xl border border-white/10 bg-black/20 p-3 text-sm"><div className="flex justify-between"><span className="text-zinc-500">FIPE</span><strong>{money(num(fipe))}</strong></div><div className="flex justify-between"><span className="text-zinc-500">Mercado base</span><strong>{money(calc.base)}</strong></div><div className="flex justify-between"><span className="text-zinc-500">Ajuste oferta × demanda</span><strong className={calc.marketPriceAdjustment<0?'text-red-200':calc.marketPriceAdjustment>0?'text-emerald-300':'text-zinc-300'}>{(calc.marketPriceAdjustment*100).toFixed(1)}%</strong></div><div className="flex justify-between"><span className="text-zinc-500">Venda recomendada</span><strong className="text-cyan-200">{money(calc.sell)}</strong></div></div>
      </section>
     ...
     <aside className="space-y-4 xl:sticky xl:top-[110px] xl:self-start">
      <section className="rounded-2xl border border-emerald-300/25 bg-emerald-300/[.045] p-5"><p className="text-[10px] font-black uppercase tracking-[.16em] text-zinc-400">MOTYQ BUY SCORE</p><div className="mt-2 flex items-end justify-between"><strong className="text-5xl font-semibold">{calc.score}</strong><span className="rounded-full border border-white/10 px-2 py-1 text-[9px] font-black text-emerald-300">{riskLabel(calc.score)}</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-black/30"><div className="h-full rounded-full bg-emerald-300" style={{width:`${calc.score}%`}}/></div><div className="mt-3 flex justify-between text-[10px] text-zinc-500"><span>Mercado {Math.round(calc.marketBalanceScore)}/100</span><span>Risco de giro {turnoverLabel(calc.marketBalanceScore)}</span></div></section>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/[.025] p-4"><p className="text-[10px] font-bold uppercase tracking-[.13em] text-zinc-500">Resumo decisório</p><div className="mt-3 space-y-2 text-sm"><div className="flex justify-between"><span className="text-zinc-500">Venda recomendada</span><strong>{money(calc.sell)}</strong></div><div className="flex justify-between"><span className="text-zinc-500">Preparação</span><strong>{money(calc.prep)}</strong></div><div className="flex justify-between"><span className="text-zinc-500">Giro esperado</span><strong>{Math.round(num(expectedDays))} dias</strong></div><div className="flex justify-between"><span className="text-zinc-500">Oferta × demanda</span><strong>{Math.round(calc.marketBalanceScore)}/100</strong></div><div className="flex justify-between"><span className="text-zinc-500">Margem estimada</span><strong className="text-emerald-300">{calc.marginPctReal.toFixed(1)}%</strong></div></div></section>

      <section className="grid gap-2">
       <button onClick={()=>window.dispatchEvent(new CustomEvent('motyq:marketiq-save-requested',{detail:{plate,vehicle,year,km,fipe,notes,marketBalanceScore:calc.marketBalanceScore,marketSupply,marketDemand,marketCompetition}}))} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-cyan-500 font-semibold text-black hover:bg-cyan-400"><Save size={16}/>SALVAR AVALIAÇÃO</button>
       <div className="grid grid-cols-2 gap-2"><button onClick={()=>window.dispatchEvent(new CustomEvent('motyq:marketiq-approved',{detail:{plate,value:calc.recommendedBuy,score:calc.score,marketBalanceScore:calc.marketBalanceScore}}))} className="flex h-10 items-center justify-center gap-2 rounded-xl border border-emerald-300/20 bg-emerald-300/[.05] text-sm font-semibold text-emerald-300"><CheckCircle2 size={15}/>APROVAR</button><button onClick={()=>window.dispatchEvent(new CustomEvent('motyq:marketiq-rejected',{detail:{plate}}))} className="flex h-10 items-center justify-center gap-2 rounded-xl border border-red-300/20 bg-red-300/[.04] text-sm font-semibold text-red-300"><XCircle size={15}/>RECUSAR</button></div>
       <p className="px-1 text-[10px] leading-4 text-zinc-600">O MarketScan atualiza o score com oferta, demanda estimada e pressão competitiva. A decisão final permanece com o avaliador.</p>
      </section>
     </aside>
    </div>
   </div>
