...
const mapPerformanceRowByPosition = (row: any[]): OperationalPerformanceSeller | null => {
  ...
  return {
    seller, sellerKey: normalize(seller), passages: toNumber(row[1]), orders: toNumber(row[2]), flowTotal: toNumber(row[3]),
    orderPercent: toPercent(row[4]), workInPeriod: toNumber(row[5]), avgContactsPerDay: toNumber(row[6]), evaluations: toNumber(row[7]),
    evaluationRate: toPercent(row[8]), closing: toNumber(row[9]), syonetSales: toNumber(row[10]), closingPercent: toPercent(row[11]),
    marginPerCar: toNumber(row[12]), marginTotal: toNumber(row[13]), marginPercent: toPercent(row[14]), captureQty: toNumber(row[15]),
    capturePercent: toPercent(row[16]), pipeline: toNumber(row[17]), projection: toNumber(row[18]), additionalPurchase: toNumber(row[19]),
  };
};
...
const readPerformanceMap = async (file: File, referenceDate: string): Promise<OperationalPerformanceSnapshot> => {
  ...
  for (let index = headerIndex + 1; index < matrix.length; index++) {
    ...
    if (first === 'indicadores da loja' || first === 'indicadores do dia') { storeSectionIndex = index; break; }
    if (!first) continue;
    const mapped = mapPerformanceRowByPosition(row);
    if (!mapped) continue;
    if (mapped.sellerKey === 'total') total = mapped; else sellers.push(mapped);
  }
...
const OperationalDataPanel: React.FC<Props> = ({ currentUser, storeId, storeName }) => {
  ...
  const stockPreview = useMemo(() => !rows.length ? [] : mapStockRows(rows, referenceDate).slice(0, 4), [rows, referenceDate]);
  ...
  const handleFile = async (next: File | null) => {
    setFile(next); setRows([]); setPerformance(null); setMessage(null);
    if (!next) return;
    try {
      const lower = next.name.toLowerCase();
      if (!lower.endsWith('.csv') && !lower.endsWith('.xls') && !lower.endsWith('.xlsx')) throw new Error('Envie CSV, XLS ou XLSX.');
      if (type === 'performance') {
        if (lower.endsWith('.csv')) throw new Error('Para o Mapa de Indicadores, envie XLS ou XLSX com as abas mensais.');
        ...
        setRows(parsed);
        if (!parsed.length) throw new Error('Não encontrei uma tabela reconhecível no arquivo.');
      }
    } catch (error: any) { setMessage({ kind: 'error', text: error?.message || 'Não consegui ler esse arquivo.' }); }
  };
  ...
  const importData = async () => {
    ...
    try {
      if (type === 'stock') {
        ...
        setMessage({ kind: 'ok', text: `${storeName}: performance atualizada com ${count} vendedores · ${linked} usuário(s) vinculados ao My Performance.` });
      }
      setFile(null); setRows([]); setPerformance(null);
      window.dispatchEvent(new Event('dealmaster:operational-data-updated'));
    } catch (error: any) { setMessage({ kind: 'error', text: error?.message || 'Erro ao importar.' }); }
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[220] overflow-y-auto bg-black/75 p-3 backdrop-blur-md" onClick={() => setOpen(false)}><div onClick={event => event.stopPropagation()} className="mx-auto mt-5 max-w-4xl rounded-[32px] border border-white/10 bg-zinc-950 shadow-2xl">
      <div className="flex items-center justify-between border-b border-white/10 p-6"><div><p className="text-xs font-semibold uppercase tracking-[.15em] text-zinc-500">Atualização diária · Multi-Store</p><h3 className="mt-1 text-2xl font-semibold text-white">{storeName}</h3><p className="mt-1 text-sm text-zinc-500">Tudo que for importado agora fica isolado nesta unidade.</p></div><button onClick={() => setOpen(false)} className="grid h-10 w-10 place-items-center rounded-full bg-white/[.06] text-zinc-400"><X size={18}/></button></div>
      <div className="p-6">
        <div className="grid gap-3 md:grid-cols-2"><button onClick={() => reset('stock')} className={`rounded-[24px] border p-5 text-left ${type === 'stock' ? 'border-white bg-white text-black' : 'border-white/10 bg-white/[.03] text-white'}`}><p className="text-xs font-semibold uppercase opacity-60">Arquivo 1</p><p className="mt-2 text-xl font-semibold">📦 Atualizar Estoque</p><p className="mt-2 text-sm opacity-60">Somente linhas visíveis do relatório filtrado.</p></button><button onClick={() => reset('performance')} className={`rounded-[24px] border p-5 text-left ${type === 'performance' ? 'border-white bg-white text-black' : 'border-white/10 bg-white/[.03] text-white'}`}><p className="text-xs font-semibold uppercase opacity-60">Arquivo 2</p><p className="mt-2 text-xl font-semibold">📊 Atualizar Performance</p><p className="mt-2 text-sm opacity-60">Fechamento oficial, MC, captura, projeção e My Performance.</p></button></div>
        <div className="mt-5 grid gap-4 md:grid-cols-[220px_1fr]"><div><label className="mb-2 block text-xs text-zinc-500">Data de referência</label><input type="date" value={referenceDate} onChange={event => setReferenceDate(event.target.value)} className="h-12 w-full rounded-2xl border border-white/10 bg-white/[.05] px-4 text-white"/></div><div><label className="mb-2 block text-xs text-zinc-500">Arquivo DealerNet / LibreOffice</label><label className="flex h-12 cursor-pointer items-center justify-between rounded-2xl border border-dashed border-white/15 bg-white/[.035] px-4 text-sm text-zinc-300"><span className="truncate">{file?.name || 'Selecionar CSV, XLS ou XLSX'}</span><FileUp size={17}/><input type="file" accept=".csv,.xls,.xlsx" className="hidden" onChange={event => handleFile(event.target.files?.[0] || null)}/></label></div></div>
        {recognized && <div className="mt-5 rounded-[24px] border border-white/10 bg-white/[.03] p-4"><div className="flex items-center justify-between gap-4"><div><p className="text-sm font-medium text-white">Arquivo reconhecido para {storeName}</p><p className="mt-1 text-xs text-zinc-500">{type === 'stock' ? `${rows.length} linhas visíveis` : `${performance?.sellers.length || 0} vendedores · aba ${performance?.sheetName}`}</p></div><span className="rounded-full bg-emerald-500/10 px-3 py-1 text-xs text-emerald-400">Prévia OK</span></div>{type === 'stock' && <div className="mt-4 grid grid-cols-2 gap-2 text-xs md:grid-cols-4">{stockPreview.map(item => <div key={item.id} className="rounded-xl bg-black/20 p-2 text-zinc-300">{item.plate || '-'}<br/><span className="text-zinc-500">{item.vehicle} · {item.stockDays} dias</span></div>)}</div>}</div>}
