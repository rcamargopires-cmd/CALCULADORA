...
const OperationalDataPanel: React.FC<Props> = ({ currentUser }) => {
  ...
  const handleFile = async (next: File | null) => {
    ...
    try {
      ...
      if (!lower.endsWith('.csv') && !lower.endsWith('.xls') && !lower.endsWith('.xlsx')) {
        setMessage({ kind: 'error', text: 'Envie CSV, XLS ou XLSX.' }); return;
      }
      if (type === 'performance') {
        if (lower.endsWith('.csv')) { setMessage({ kind: 'error', text: 'Para o Mapa de Indicadores, envie XLS ou XLSX com as abas mensais.' }); return; }
  ...
  const reset = (t: ImportType) => { setType(t); setFile(null); setRows([]); setPerformance(null); setMessage(null); };
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[220] overflow-y-auto bg-black/75 p-3 backdrop-blur-md" onClick={() => setOpen(false)}><div onClick={e => e.stopPropagation()} className="mx-auto mt-5 max-w-4xl rounded-[32px] border border-white/10 bg-zinc-950 shadow-2xl">
      <div className="flex items-center justify-between border-b border-white/10 p-6"><div><p className="text-xs font-semibold uppercase tracking-[.15em] text-zinc-500">Atualização diária</p><h3 className="mt-1 text-2xl font-semibold text-white">Dados do Outlet</h3><p className="mt-1 text-sm text-zinc-500">O estoque respeita as linhas visíveis do arquivo; o mapa preserva o histórico.</p></div><button onClick={() => setOpen(false)} className="grid h-10 w-10 place-items-center rounded-full bg-white/[.06] text-zinc-400"><X size={18}/></button></div>
      <div className="p-6">
        <div className="grid gap-3 md:grid-cols-2"><button onClick={() => reset('stock')} className={`rounded-[24px] border p-5 text-left ${type === 'stock' ? 'border-white bg-white text-black' : 'border-white/10 bg-white/[.03] text-white'}`}><p className="text-xs font-semibold uppercase opacity-60">Arquivo 1</p><p className="mt-2 text-xl font-semibold">📦 Atualizar Estoque</p><p className="mt-2 text-sm opacity-60">Considera somente as linhas visíveis do relatório filtrado.</p></button><button onClick={() => reset('performance')} className={`rounded-[24px] border p-5 text-left ${type === 'performance' ? 'border-white bg-white text-black' : 'border-white/10 bg-white/[.03] text-white'}`}><p className="text-xs font-semibold uppercase opacity-60">Arquivo 2</p><p className="mt-2 text-xl font-semibold">📊 Atualizar Performance</p><p className="mt-2 text-sm opacity-60">Mapa de Indicadores: Fechamento, margem, captura, projeção e indicadores da loja.</p></button></div>
        <div className="mt-5 grid gap-4 md:grid-cols-[220px_1fr]"><div><label className="mb-2 block text-xs text-zinc-500">Data de referência</label><input type="date" value={referenceDate} onChange={e => setReferenceDate(e.target.value)} className="h-12 w-full rounded-2xl border border-white/10 bg-white/[.05] px-4 text-white"/></div><div><label className="mb-2 block text-xs text-zinc-500">Arquivo do DealerNet / LibreOffice</label><label className="flex h-12 cursor-pointer items-center justify-between rounded-2xl border border-dashed border-white/15 bg-white/[.035] px-4 text-sm text-zinc-300"><span className="truncate">{file?.name || 'Selecionar CSV, XLS ou XLSX'}</span><FileUp size={17}/><input type="file" accept=".csv,.xls,.xlsx" className="hidden" onChange={e => handleFile(e.target.files?.[0] || null)}/></label></div></div>
        {recognized && <div className="mt-5 rounded-[24px] border border-white/10 bg-white/[.03] p-4"><div className="flex justify-between"><div><p className="text-sm font-medium text-white">Arquivo reconhecido</p><p className="text-xs text-zinc-500">{type === 'stock' ? `${rows.length} veículos/linhas visíveis · mapeamento pronto` : `${performance?.sellers.length || 0} vendedores · aba ${performance?.sheetName}`}</p></div><span className="rounded-full bg-emerald-500/10 px-3 py-1 text-xs text-emerald-400">Prévia OK</span></div>{type === 'stock' && <div className="mt-4 grid grid-cols-2 gap-2 text-xs md:grid-cols-4">{stockPreview.map((i: any) => <div key={i.id} className="rounded-xl bg-black/20 p-2 text-zinc-300">{i.plate || '-'}<br/><span className="text-zinc-500">{i.vehicle} · {i.stockDays} dias</span></div>)}</div>}{type === 'performance' && performance && <div className="mt-4 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-3">{performance.sellers.slice(0, 6).map(s => <div key={s.sellerKey} className="rounded-xl bg-black/20 p-3"><p className="font-medium text-zinc-200">{s.seller}</p><p className="mt-1 text-zinc-500">Vendas/Fechamento: <span className="text-zinc-300">{s.closing}</span> · Projeção: <span className="text-zinc-300">{s.projection.toFixed(1)}</span></p><p className="mt-1 text-zinc-500">MC: <span className="text-zinc-300">{s.marginPercent.toFixed(1)}%</span> · Captura: <span className="text-zinc-300">{s.capturePercent.toFixed(1)}%</span></p></div>)}</div>}</div>}
