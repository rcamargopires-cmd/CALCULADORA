  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[235] overflow-y-auto bg-black/80 p-3 backdrop-blur-md md:p-6" onClick={() => setOpen(false)}><div className="mx-auto w-full max-w-6xl overflow-hidden rounded-[34px] border border-white/10 bg-zinc-950 shadow-2xl" onClick={event => event.stopPropagation()}>
      ...
      <div className="space-y-6 p-5 md:p-6">
        ...
        <section className="rounded-[28px] border border-white/10 bg-white/[0.035] p-5"><div className="flex items-end justify-between gap-4"><div><div className="flex items-center gap-2 text-zinc-500"><Users size={16}/><p className="text-xs font-semibold uppercase tracking-[0.13em]">Acesso por empresa</p></div><h4 className="mt-1 text-lg font-semibold text-white">Vincular usuários</h4></div><p className="hidden text-xs text-zinc-600 md:block">Plano e módulos acompanham o usuário sem expor a carteira de clientes.</p></div><div className="mt-4 grid gap-3 lg:grid-cols-2">{users.filter(user => user.role !== 'admin').map(user => <div key={user.id} className="flex flex-col gap-3 rounded-[22px] border border-white/10 bg-black/20 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium text-white">{user.name}</p><p className="mt-1 text-xs text-zinc-500">{user.email} · {user.role === 'manager' ? 'Gestor' : 'Vendedor'}</p></div><select disabled={saving === user.id} value={companyIdForUser(user)} onChange={event => assignUser(user, event.target.value)} className="h-10 min-w-48 rounded-xl border border-white/10 bg-zinc-900 px-3 text-sm text-zinc-200 outline-none">{companies.filter(company => company.status !== 'suspended').map(company => <option key={company.id} value={company.id}>{company.name}</option>)}</select></div>)}</div></section>

        <section className="rounded-[24px] border border-violet-400/15 bg-violet-400/[0.055] p-4"><div className="flex gap-3"><ShieldCheck size={17} className="mt-0.5 shrink-0 text-violet-300"/><div><p className="text-sm font-medium text-violet-200">Camada comercial ativa</p><p className="mt-1 text-xs leading-5 text-violet-200/60">Planos definem módulos por padrão e permitem exceções por cliente. Usuários recebem um snapshot do pacote contratado, mantendo o isolamento entre empresas.</p></div></div></section>
