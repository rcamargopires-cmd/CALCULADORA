...
import { Building, CheckCircle2, ChevronRight, Plus, Presentation, ShieldCheck, Sparkles, Users, X } from 'lucide-react';
...
import { enabledModules, PLAN_META } from '../services/planEntitlementService';
...
const CompaniesPanel: React.FC<{ currentUser: User }> = ({ currentUser }) => {
  ...
  const openDemo = async () => {
    ...
    try {
      ...
      await load();
      setSelectedCompanyId(DEMO_COMPANY_ID);
      setMessage('Motyq Demo Motors preparada. Estoque, performance, negociações e ações já estão carregados para apresentação.');
    } catch (error) {
      console.error('Erro ao preparar ambiente demo:', error);
      setMessage('Não foi possível preparar a demo agora. Tente novamente em instantes.');
    } finally {
      setSaving(null);
    }
  };
  ...
  const updateCompany = async (company: Company, patch: Partial<Company>) => {
    ...
    try {
      ...
      setMessage(`${company.name}: plano e acessos atualizados para ${affected.length} usuário(s).`);
    } finally { setSaving(null); }
  };
  ...
  const assignUser = async (user: User, companyId: string) => {
    ...
    const targetCompany = companies.find(company => company.id === companyId) || DEFAULT_COMPANY;
    setSaving(user.id);
    try {
      const next = {
        ...user,
        companyId,
        ...(storeId ? { storeId } : {}),
        companyPlan: targetCompany.plan,
        companyStatus: targetCompany.status,
        companyModuleOverrides: targetCompany.moduleOverrides,
      };
      await userService.save(next);
      setUsers(prev => prev.map(item => item.id === user.id ? next : item));
      setMessage(`${user.name} agora pertence a ${companyService.getName(companies, companyId)} com o pacote ${planLabel[targetCompany.plan]}.`);
    } finally { setSaving(null); }
  };
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[235] overflow-y-auto bg-black/80 p-3 backdrop-blur-md md:p-6" onClick={() => setOpen(false)}><div className="mx-auto w-full max-w-6xl overflow-hidden rounded-[34px] border border-white/10 bg-zinc-950 shadow-2xl" onClick={event => event.stopPropagation()}>
      <header className="flex items-center justify-between border-b border-white/10 p-5 md:p-6"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-black"><Building size={21}/></div><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">SaaS Foundation</p><h3 className="mt-1 text-xl font-semibold text-white">Empresas clientes</h3><p className="mt-1 text-xs text-zinc-500">Empresa → plano → módulos → unidades → usuários → dados.</p></div></div><button onClick={() => setOpen(false)} className="grid h-10 w-10 place-items-center rounded-full bg-white/[0.06] text-zinc-400"><X size={18}/></button></header>

      <div className="space-y-6 p-5 md:p-6">
        <section className="overflow-hidden rounded-[28px] border border-sky-300/20 bg-gradient-to-br from-sky-300/[0.09] via-white/[0.035] to-violet-400/[0.06] p-5 md:p-6">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div className="flex gap-4">
              <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl border border-sky-300/20 bg-sky-300/[0.08] text-sky-300"><Presentation size={22}/></div>
              <div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-sky-300">MODO DEMO COMERCIAL</p><h4 className="mt-1 text-xl font-semibold text-white">Apresente o Motyq em 5 minutos</h4><p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">Cria ou recompõe um ambiente fictício isolado com 4 vendedores, estoque envelhecido, performance em evolução, negociações abertas, tarefas e Impacto Motyq. Nenhum dado real é alterado.</p></div>
            </div>
            <button onClick={openDemo} disabled={saving === 'demo'} className="flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-2xl bg-sky-300 px-5 text-sm font-black text-sky-950 shadow-lg shadow-sky-950/20 transition hover:bg-sky-200 disabled:opacity-50"><Sparkles size={17}/>{saving === 'demo' ? 'PREPARANDO DEMO...' : selectedCompanyId === DEMO_COMPANY_ID ? 'RECOMPOR DEMO' : 'PREPARAR E ABRIR DEMO'}</button>
          </div>
        </section>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
          <div className="rounded-[28px] border border-white/10 bg-white/[0.035] p-5"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.13em] text-zinc-500">Carteira</p><h4 className="mt-1 text-lg font-semibold text-white">Ambientes cadastrados</h4></div><span className="text-xs text-zinc-600">{companies.length} empresa(s)</span></div><div className="mt-4 grid gap-3 md:grid-cols-2">{cards.map(({ company, stores: companyStores, users: companyUsers }) => {
            ...
            const activeModules = enabledModules(company).length;
            return <div key={company.id} className={`rounded-[23px] border p-4 ${selected ? 'border-white/25 bg-white/[0.07]' : 'border-white/10 bg-black/20'}`}><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-white">{company.name}</p>{isDemo && <span className="rounded-full border border-sky-300/20 bg-sky-300/10 px-2 py-0.5 text-[10px] font-black text-sky-300">DEMO</span>}{selected && <span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-bold text-black">Visualizando</span>}</div><p className="mt-1 text-xs text-zinc-500">{planLabel[company.plan]} · {isDemo ? 'ambiente comercial' : `${money(PLAN_META[company.plan].price)}/mês`} · {company.status === 'trial' ? 'Avaliação' : company.status === 'suspended' ? 'Suspensa' : 'Ativa'}</p></div><button onClick={() => viewCompany(company)} disabled={company.status === 'suspended'} className="grid h-9 w-9 place-items-center rounded-full bg-white/[0.06] text-zinc-400 disabled:opacity-30"><ChevronRight size={16}/></button></div><div className="mt-4 grid grid-cols-3 gap-2"><Info label="Unidades" value={`${companyStores.filter(store => store.active).length}`}/><Info label="Usuários" value={`${companyUsers.length}`}/><Info label="Módulos" value={`${activeModules}/13`}/></div><div className="mt-3 flex flex-wrap gap-2"><CompanyPlanEditor company={company} locked={company.id === DEFAULT_COMPANY_ID || isDemo} saving={saving === company.id} onSave={patch => updateCompany(company, patch)}/>{company.id !== DEFAULT_COMPANY_ID && !isDemo && <button disabled={saving === company.id} onClick={() => updateCompany(company, { status: company.status === 'suspended' ? 'active' : 'suspended' })} className="h-9 rounded-xl border border-white/10 px-3 text-xs text-zinc-400">{company.status === 'suspended' ? 'Reativar' : 'Suspender'}</button>}</div></div>})}</div></div>

          <div className="rounded-[28px] border border-white/10 bg-gradient-to-br from-zinc-900 to-black p-5"><div className="flex items-center gap-2 text-zinc-500"><Plus size={16}/><p className="text-xs font-semibold uppercase tracking-[0.13em]">Novo cliente</p></div><h4 className="mt-2 text-lg font-semibold text-white">Criar ambiente</h4><p className="mt-1 text-sm leading-6 text-zinc-500">A empresa nasce com uma unidade principal e avaliação de 14 dias.</p><label className="mt-5 block"><span className="text-xs text-zinc-500">Empresa / grupo</span><input value={name} onChange={event => setName(event.target.value)} placeholder="Ex.: Grupo Automax" className="mt-2 h-11 w-full rounded-2xl border border-white/10 bg-white/[0.04] px-4 text-sm text-white outline-none"/></label><div className="mt-3 grid gap-2">{(['starter','pro','enterprise'] as CompanyPlan[]).map(item => <button key={item} onClick={() => setPlan(item)} className={`rounded-2xl border p-3 text-left ${plan === item ? 'border-white/30 bg-white/[0.08]' : 'border-white/10 bg-white/[0.025]'}`}><div className="flex items-center justify-between"><span className="text-sm font-semibold text-white">{planLabel[item]}</span>{plan === item && <CheckCircle2 size={15} className="text-emerald-300"/>}</div><p className="mt-1 text-xs text-zinc-600">{planHint[item]} · {money(PLAN_META[item].price)}/mês</p></button>)}</div><button disabled={!name.trim() || saving === 'new'} onClick={createCompany} className="mt-4 flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-white text-sm font-semibold text-black disabled:opacity-30"><Sparkles size={16}/>{saving === 'new' ? 'Criando...' : 'Criar empresa'}</button></div>
        </section>

        <section className="rounded-[28px] border border-white/10 bg-white/[0.035] p-5"><div className="flex items-end justify-between gap-4"><div><div className="flex items-center gap-2 text-zinc-500"><Users size={16}/><p className="text-xs font-semibold uppercase tracking-[0.13em]">Acesso por empresa</p></div><h4 className="mt-1 text-lg font-semibold text-white">Vincular usuários</h4></div><p className="hidden text-xs text-zinc-600 md:block">Plano e módulos acompanham o usuário sem expor a carteira de clientes.</p></div><div className="mt-4 grid gap-3 lg:grid-cols-2">{users.filter(user => user.role !== 'admin' && companyIdForUser(user) !== DEMO_COMPANY_ID).map(user => <div key={user.id} className="flex flex-col gap-3 rounded-[22px] border border-white/10 bg-black/20 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium text-white">{user.name}</p><p className="mt-1 text-xs text-zinc-500">{user.email} · {user.role === 'manager' ? 'Gestor' : 'Vendedor'}</p></div><select disabled={saving === user.id} value={companyIdForUser(user)} onChange={event => assignUser(user, event.target.value)} className="h-10 min-w-48 rounded-xl border border-white/10 bg-zinc-900 px-3 text-sm text-zinc-200 outline-none">{companies.filter(company => company.status !== 'suspended' && company.id !== DEMO_COMPANY_ID).map(company => <option key={company.id} value={company.id}>{company.name}</option>)}</select></div>)}</div></section>

        <section className="rounded-[24px] border border-violet-400/15 bg-violet-400/[0.055] p-4"><div className="flex gap-3"><ShieldCheck size={17} className="mt-0.5 shrink-0 text-violet-300"/><div><p className="text-sm font-medium text-violet-200">Camada comercial ativa</p><p className="mt-1 text-xs leading-5 text-violet-200/60">Planos definem módulos por padrão e permitem exceções por cliente. Usuários recebem um snapshot do pacote contratado, mantendo o isolamento entre empresas. O tenant demo fica bloqueado contra alterações acidentais de plano.</p></div></div></section>
