...
const StockHub: React.FC<Props> = ({ canStockIntelligence, canAssetGuard, storeName }) => {
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[562] bg-black/70 backdrop-blur-sm" onClick={() => setOpen(false)}>
      <aside className="ml-auto flex min-h-full w-full max-w-[470px] flex-col border-l border-white/10 bg-[#151411] text-white shadow-2xl" onClick={event => event.stopPropagation()}>
