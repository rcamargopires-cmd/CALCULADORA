...
import { CalendarDays, CarFront, Clock3, History, Phone, Search, UserRound, X } from 'lucide-react';
...
const SLOT_ID = 'motyq-seller-showroom-history-slot';
...
const SellerShowroomHistory: React.FC<Props> = ({ user }) => {
  ...
  useEffect(() => {
    if (!companyId || !storeId || !user.email) return;
    return showroomFlowService.subscribeSellerPassages(
      ...
      err => {
        console.error('Seller showroom history error', err);
        setError('Não foi possível carregar seu histórico de atendimentos agora.');
      },
    );
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(item => {
      ...
      const searchOk = !needle || [item.customerName, item.phone, item.interestModel, item.notes, STATUS[item.status]]
        .some(value => String(value || '').toLocaleLowerCase('pt-BR').includes(needle));
      return periodOk && originOk && searchOk;
    });
  }, [items, period, origin, search]);
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[590] overflow-y-auto bg-slate-950/55 p-3 backdrop-blur-sm md:p-6" onClick={() => setOpen(false)}>
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[30px] border border-slate-200 bg-[#f7f9fc] shadow-2xl" onClick={event => event.stopPropagation()}>
        <header className="flex items-start justify-between gap-4 border-b border-slate-200 bg-white p-5 md:p-7">
          <div className="flex gap-3">
            <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-sky-200 bg-sky-50 text-sky-700"><History size={20}/></div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-sky-700">SHOWROOMFLOW · VENDEDOR</p>
              <h2 className="mt-1 text-2xl font-semibold text-slate-900">Meu histórico de atendimentos</h2>
              <p className="mt-1 text-sm text-slate-500">Clientes atribuídos ao seu login na recepção, com passagem, pedido e interesse registrado.</p>
            </div>
          </div>
        ...
        <div className="space-y-5 p-5 md:p-7">
          <section className="grid gap-3 md:grid-cols-3">
            ...
            <label className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
              ...
              <input value={search} onChange={event => setSearch(event.target.value)} placeholder="Nome, telefone, carro, observação..." className="mt-2 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-800 outline-none focus:border-sky-400"/>
            </label>
          </section>
          ...
          <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm">
            <div className="hidden grid-cols-[120px_1.15fr_1fr_115px_1.3fr_130px] gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400 lg:grid">
              <span>Data / hora</span><span>Cliente</span><span>Telefone</span><span>Origem</span><span>Procurava</span><span>Status</span>
            </div>
            ...
            <div className="max-h-[62vh] overflow-y-auto">
              {!filtered.length ? (
                <div className="p-12 text-center">
                  ...
                  <p className="mt-1 text-xs text-slate-400">Altere o período ou a busca para consultar outros registros.</p>
                </div>
              ) : filtered.map(item => (
                <article key={item.id} className="border-b border-slate-100 px-4 py-4 last:border-0">
                  <div className="grid gap-4 lg:grid-cols-[120px_1.15fr_1fr_115px_1.3fr_130px] lg:items-center">
                    <div>
...
export default SellerShowroomHistory;
