...
const AIManagerV2: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, user => setLogged(Boolean(user)));
    return unsubscribe;
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      unsubscribeDeals = onSnapshot(
        query(collection(db, 'deals'), orderBy('timestamp', 'desc')),
        snapshot => setDeals(snapshot.docs.map(item => ({ id: item.id, ...item.data() } as SavedCalculation))),
        error => console.warn('[AI Manager V2] deals indisponível:', error)
      );

      unsubscribePerformance = onSnapshot(
        doc(db, 'config/performance'),
        snapshot => {
          if (snapshot.exists()) setPerformance({ ...DEFAULTS, ...(snapshot.data() as Partial<PerformanceConfig>) });
        },
        error => console.warn('[AI Manager V2] metas indisponíveis:', error)
      );
    } catch (error) {
      console.warn('[AI Manager V2] inicialização ignorada:', error);
    }
  ...
  const data = useMemo(() => {
    ...
    const month = deals.filter(d => d.timestamp?.startsWith(prefix));
    const closed = month.filter(d => d.data?.dealStatus === 'closed');
    const openDeals = deals.filter(d => d.data?.dealStatus !== 'closed');
  ...
  const priorities = useMemo<Priority[]>(() => {
    ...
    if (data.critical.length) items.push({
      id: 'stock',
      level: 'critical',
      title: 'Destravar estoque crítico',
    ...
    if (data.capture < performance.captureGoal && data.actual > 0) items.push({
      id: 'capture',
      level: performance.captureGoal - data.capture >= 10 ? 'critical' : 'warning',
      title: 'Elevar captura',
      reason: `Captura em ${data.capture.toFixed(0)}%, meta ${performance.captureGoal}%.`,
    ...
    if (data.avgMargin < performance.healthyMargin && data.actual > 0) items.push({
      id: 'margin',
      level: data.avgMargin < performance.healthyMargin - 2 ? 'critical' : 'warning',
      title: 'Proteger margem',
      reason: `Margem média em ${data.avgMargin.toFixed(1)}%, referência ${performance.healthyMargin}%.`,
    ...
    const weak = data.sellers
      .filter(s => s.projection < performance.sellerMonthlyGoal || s.capture < performance.captureGoal)
    ...
    if (weak.length) items.push({
      id: 'team',
      level: 'warning',
      title: 'Foco individual na equipe',
    ...
    if (!items.length) items.push({
      id: 'healthy', level: 'good', title: 'Operação equilibrada',
      reason: 'Ritmo, captura, margem e estoque estão dentro das referências.',
      action: 'Mantenha disciplina comercial e preserve margem.', metric: 'Sem alerta'
    });
      ...
      <div className="mx-auto flex max-h-[94vh] w-full max-w-4xl flex-col overflow-hidden rounded-[32px] border border-white/10 bg-zinc-950 shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-white/10 p-5 md:p-6">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">DealMaster AI Manager V2</p><h3 className="mt-1 text-xl font-semibold text-white">Plano de ação do dia</h3></div>
          </div>
        ...
        <div className="overflow-y-auto p-5 md:p-6">
          <div className={`rounded-[24px] border p-5 ${health.tone === 'critical' ? 'border-red-500/20 bg-red-500/[0.08]' : health.tone === 'warning' ? 'border-amber-400/20 bg-amber-400/[0.08]' : 'border-emerald-500/20 bg-emerald-500/[0.08]'}`}>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
              <ManagerMetric label="Projeção" value={`${data.projection}/${performance.monthlyGoal}`} ok={data.projection >= performance.monthlyGoal}/>
              <ManagerMetric label="Captura" value={`${data.capture.toFixed(0)}%`} ok={data.capture >= performance.captureGoal}/>
              <ManagerMetric label="Margem" value={`${data.avgMargin.toFixed(1)}%`} ok={data.avgMargin >= performance.healthyMargin}/>
              <ManagerMetric label="Estoque +90d" value={`${data.critical.length}`} ok={!data.critical.length}/>
            </div>
          </div>

          <div className="mt-5 space-y-3">
            {priorities.map((p, index) => <div key={p.id} className="rounded-[24px] border border-white/10 bg-white/[0.035] p-4 md:p-5">
              <div className="flex items-start gap-4">
                <div className={`grid h-10 w-10 shrink-0 place-items-center rounded-full ${p.level === 'critical' ? 'bg-red-500/12 text-red-400' : p.level === 'warning' ? 'bg-amber-400/12 text-amber-400' : 'bg-emerald-500/12 text-emerald-400'}`}>
                  {p.level === 'critical' ? <ShieldAlert size={17}/> : p.level === 'warning' ? <AlertTriangle size={17}/> : <CheckCircle2 size={17}/>} 
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap items-center justify-between gap-2"><h4 className="font-medium text-white">{index + 1}. {p.title}</h4><span className="rounded-full bg-white/[0.05] px-2.5 py-1 text-[11px] text-zinc-400">{p.metric}</span></div>
                  <p className="mt-2 text-xs leading-5 text-zinc-500">{p.reason}</p>
                  <div className="mt-3 flex items-start gap-2 text-sm leading-5 text-zinc-300"><Crosshair size={15} className="mt-0.5 shrink-0 text-zinc-600"/><span>{p.action}</span></div>
                </div>
              </div>
            </div>)}
          </div>

          <div className="mt-5 flex items-center gap-2 rounded-2xl bg-white/[0.03] p-4 text-xs leading-5 text-zinc-500"><Sparkles size={15} className="shrink-0"/> O AI Manager roda isolado. Se ele falhar, o restante do DealMaster continua funcionando.</div>
        </div>
      </div>
    </div>}
  </>;
