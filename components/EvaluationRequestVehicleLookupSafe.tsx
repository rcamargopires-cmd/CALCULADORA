...
const hideAutomaticFields = () => {
  ...
  const hidden = ['ano/modelo', 'modelo / versão', 'km atual'];
  labels.forEach(label => {
    const text = String(label.textContent || '').trim().toLowerCase();
    if (hidden.some(item => text.startsWith(item))) label.style.display = 'none';
  });
};

type Notice = { kind: 'loading' | 'ok' | 'warn'; text: string } | null;

const EvaluationRequestVehicleLookupSafe: React.FC = () => {
  ...
  useEffect(() => {
    if (!user) return;
    return groupStockService.subscribe(companyIdForUser(user), setSnapshot, () => setSnapshot(null));
  }, [user]);

  useEffect(() => {
    const id = window.setInterval(() => {
      if (requestRoot()) hideAutomaticFields();
      else {
  ...
  useEffect(() => {
    ...
    const applyHistoryFallback = async (companyId: string, storeId: string, plate: string, currentRequest: number) => {
      const previous = await marketIqEvaluationService.getLatestByPlate(companyId, storeId, plate).catch(() => null);
      if (currentRequest !== requestRef.current || !previous) return false;
      setInput(field('Modelo / versão'), previous.vehicle || '');
      setInput(field('Ano/modelo'), previous.year || '');
      setInput(field('KM atual'), previous.km || '');
      setNotice({ kind: 'ok', text: `${previous.vehicle || plate}${previous.year ? ` · ${previous.year}` : ''} reconhecido pelo histórico do MarketIQ.` });
      return true;
    };
    ...
    const runLookup = async () => {
      ...
      try {
        ...
        const renavamMatches = Boolean(renavam && cachedRenavam && renavam === cachedRenavam);

        if (trustedCrlv && (!renavam || !cachedRenavam || renavamMatches)) {
        ...
        if (cached && renavam && cachedRenavam && renavam !== cachedRenavam) {
          setNotice({ kind: 'warn', text: `O RENAVAM informado não confere com o veículo ${plate} já conhecido pelo MOTYQ.` });
          return;
        }
        ...
        if (await applyHistoryFallback(companyId, storeId, plate, currentRequest)) return;

        setNotice({ kind: 'warn', text: 'Veículo ainda não identificado. Você pode enviar a solicitação; o avaliador concluirá a identificação no MarketIQ.' });
      } catch {
        if (currentRequest !== requestRef.current) return;
        if (await applyHistoryFallback(companyId, storeId, plate, currentRequest)) return;
        setNotice({ kind: 'warn', text: 'Consulta indisponível agora. A solicitação ainda pode ser enviada normalmente.' });
      }
    };
    ...
    const onInput = (event: Event) => {
      ...
      timerRef.current = window.setTimeout(() => void runLookup(), 400);
    };
  ...
  if (!user || !requestRoot() || !notice) return null;
...
export default EvaluationRequestVehicleLookupSafe;
