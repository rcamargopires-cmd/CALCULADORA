...
const PassageCard=({item,children}:{item:ShowroomPassage;children?:React.ReactNode})=><div className={`rounded-2xl border p-4 ${item.status==='waiting'&&mins(item.createdAt)>=10?'border-red-400/20 bg-red-400/[.04]':originOf(item)==='requested'?'border-violet-400/15 bg-violet-400/[.025]':'border-white/10 bg-black/20'}`}><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><OriginBadge origin={originOf(item)}/><p className="font-semibold text-white">{item.customerName}</p>{item.status==='waiting'&&<span className="rounded-full bg-amber-400/10 px-2 py-1 text-[10px] font-bold text-amber-300">{mins(item.createdAt)} MIN</span>}</div><p className="mt-1 text-sm text-zinc-500">{item.interestModel||'Interesse não informado'} · {phoneMask(item.phone)}</p><p className="mt-1 text-xs text-zinc-600">Vendedor: {item.assignedSellerName}</p>{originOf(item)==='requested'&&BUSY_REQUESTED_STATUSES.has(item.status)&&<p className="mt-1 text-[10px] text-violet-300">Pedido ativo · vez da passagem preservada</p>}</div><span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-bold ${item.status==='sale'?'bg-emerald-400/10 text-emerald-300':item.status==='waiting'?'bg-amber-400/10 text-amber-300':'bg-violet-400/10 text-violet-300'}`}>{STATUS[item.status]}</span></div>{children&&<div className="mt-3 flex justify-end border-t border-white/10 pt-3">{children}</div>}</div>;
