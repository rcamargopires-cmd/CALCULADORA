...
import { Building2, CarFront, ChevronRight, CircleAlert, RefreshCw, TrendingUp, WalletCards, X } from 'lucide-react';
...
type StoreRow = { store: Store; snapshot: OperationalPerformanceSnapshot | null; stock: OperationalStockItem[]; sales: number; projection: number; margin: number; capture: number; evaluations: number; stockValue: number; critical90: number; criticalValue: number };
type Props = { currentUser: User; companyId: string; companyName: string };

const GroupOverview: React.FC<Props> = ({ currentUser, companyId, companyName }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await Promise.all(active.map(async store => {
        const [snapshot, stock] = await Promise.all([storeScopedOperationalService.getLatestPerformance(store.id), storeScopedOperationalService.getLatestStock(store.id)]);
        const total = totalFromSnapshot(snapshot), sales = officialClosingCount(total);
        ...
        return { store, snapshot, stock, sales, projection: Number(total?.projection || 0), margin: Number(total?.marginPercent || 0), capture: Number(total?.capturePercent || 0), evaluations: Number(total?.evaluations || 0), stockValue, critical90: critical.length, criticalValue: critical.reduce((sum, item) => sum + Number(item.cost || 0), 0) } as StoreRow;
      }));
      setRows(data);
  ...
  const summary = useMemo(() => {
    const withData = rows.filter(row => !!row.snapshot), sales = withData.reduce((sum, row) => sum + row.sales, 0);
    const goal = withData.length * performance.monthlyGoal, projection = withData.reduce((sum, row) => sum + row.projection, 0);
    ...
    return { withData, sales, goal, projection, capture, margin, marginValue, stockCount: rows.reduce((sum, row) => sum + row.stock.length, 0), stockValue: rows.reduce((sum, row) => sum + row.stockValue, 0), critical90: rows.reduce((sum, row) => sum + row.critical90, 0), criticalValue: rows.reduce((sum, row) => sum + row.criticalValue, 0) };
  }, [rows, performance]);

  const ranking = useMemo(() => [...rows].sort((a, b) => {
    if (!!a.snapshot !== !!b.snapshot) return a.snapshot ? -1 : 1;
    return (b.snapshot ? b.projection / Math.max(performance.monthlyGoal, 1) : -1) - (a.snapshot ? a.projection / Math.max(performance.monthlyGoal, 1) : -1);
  }), [rows, performance]);

  if (currentUser.role !== 'admin') return null;
  return <>
    <button onClick={() => { setOpen(true); load(); }} className="fixed bottom-44 left-5 z-[143] flex items-center gap-2 rounded-full border border-white/10 bg-zinc-900 px-4 py-3 text-sm font-semibold text-white shadow-2xl"><Building2 size={18}/> Grupo</button>
    {open && <div className="fixed inset-0 z-[230] overflow-y-auto bg-black/80 p-3 backdrop-blur-md md:p-6" onClick={() => setOpen(false)}><div className="mx-auto w-full max-w-6xl overflow-hidden rounded-[34px] border border-white/10 bg-zinc-950 shadow-2xl" onClick={event => event.stopPropagation()}>
      <header className="flex items-center justify-between border-b border-white/10 p-5 md:p-6"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-black"><Building2 size={21}/></div><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">Group Command Center · {companyName}</p><h3 className="mt-1 text-xl font-semibold text-white">Consolidado da empresa</h3><p className="mt-1 text-xs text-zinc-500">Somente unidades pertencentes a este cliente entram no consolidado.</p></div></div><div className="flex gap-2"><button onClick={load} className="grid h-10 w-10 place-items-center rounded-full bg-white/[0.06] text-zinc-400"><RefreshCw size={17} className={loading ? 'animate-spin' : ''}/></button><button onClick={() => setOpen(false)} className="grid h-10 w-10 place-items-center rounded-full bg-white/[0.06] text-zinc-400"><X size={18}/></button></div></header>
      <div className="space-y-5 p-5 md:p-6">
        <section className="rounded-[30px] border border-white/10 bg-gradient-to-br from-zinc-800 via-zinc-900 to-black p-6 md:p-7"><div className="grid gap-6 lg:grid-cols-[1.2fr_.8fr] lg:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-zinc-500">GoalTrack · Empresa</p><div className="mt-3 flex items-end gap-3"><span className="text-6xl font-semibold tracking-[-0.06em] text-white">{summary.sales}</span><span className="pb-2 text-xl text-zinc-500">de {summary.goal}</span></div><div className="mt-5 h-2.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-white" style={{ width: `${Math.min(summary.sales / Math.max(summary.goal, 1) * 100, 100)}%` }}/></div></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Mini label="Unidades com dados" value={`${summary.withData.length}/${stores.length}`}/><Mini label="Projeção" value={summary.projection.toFixed(1)}/><Mini label="Captura" value={`${summary.capture.toFixed(1)}%`}/><Mini label="Margem MC" value={`${summary.margin.toFixed(1)}%`}/></div></div></section>
        <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><Metric icon={<TrendingUp size={18}/>} label="Vendas" value={`${summary.sales}`} hint={`Meta consolidada ${summary.goal}`}/><Metric icon={<WalletCards size={18}/>} label="Margem MC" value={`${summary.margin.toFixed(1)}%`} hint={formatCurrency(summary.marginValue)}/><Metric icon={<CarFront size={18}/>} label="Estoque" value={`${summary.stockCount}`} hint={formatCurrency(summary.stockValue)}/><Metric icon={<CircleAlert size={18}/>} label="Estoque +90" value={`${summary.critical90}`} hint={formatCurrency(summary.criticalValue)}/></section>
        <section className="rounded-[30px] border border-white/10 bg-white/[0.035] p-5 md:p-6"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-zinc-500">Ranking das unidades</p><h4 className="mt-1 text-xl font-semibold text-white">Ritmo e saúde operacional</h4></div><div className="mt-5 grid gap-3 lg:grid-cols-2">{ranking.map((row, index) => { const hasData = !!row.snapshot, coverage = hasData ? row.projection / Math.max(performance.monthlyGoal, 1) * 100 : 0; return <button key={row.store.id} onClick={() => { storeScopeService.set(row.store.id); setOpen(false); }} className="group rounded-[24px] border border-white/10 bg-black/20 p-4 text-left transition hover:border-white/20 hover:bg-white/[0.045]"><div className="flex items-start justify-between gap-4"><div className="flex gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white/[0.06] text-xs font-bold text-zinc-400">{index + 1}</div><div><p className="font-semibold text-white">{row.store.name}</p><p className="mt-1 text-xs text-zinc-500">{hasData ? `${row.sales}/${performance.monthlyGoal} vendas · projeção ${row.projection.toFixed(1)} · ${coverage.toFixed(0)}%` : 'Aguardando Mapa de Performance'}</p></div></div><ChevronRight size={17} className="mt-2 text-zinc-700"/></div>{hasData && <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><Small label="MC" value={`${row.margin.toFixed(1)}%`}/><Small label="Captura" value={`${row.capture.toFixed(1)}%`}/><Small label="Avaliações" value={`${row.evaluations}`}/><Small label="+90 dias" value={`${row.critical90}`}/></div>}</button>})}</div></section>
      </div>
    </div></div>}
  </>;
};
